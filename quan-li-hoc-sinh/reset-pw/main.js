...
function checkCondition(otp) {
  ...
  if (otp_input == "") {
    document.getElementById("error-message").innerHTML =
      "Thầy/Cô chưa nhập mã xác thực";
  } else if (otp_input != otp) {
    document.getElementById("error-message").innerHTML =
      "Thầy/ Cô nhập sai mã xác thực. Vui lòng nhập lại";
  } else {
    if (user_name == "loan_1108" && phone_number == 123456789) {
      alert(
        "Mật khẩu mới đã được gửi về số điện thoại đăng kí của quý Thầy/ cô"
      );
      MoveToLoginScreen();
    } else if (user_name == "" || phone_number == "") {
      document.getElementById("error-message").innerHTML =
        "Thầy/ cô cần nhập đầy đủ tên đăng nhập và số điện thoại";
    } else {
      document.getElementById("error-message").innerHTML =
        "Thầy/ cô vui lòng kiểm tra lại tên đăng nhập và mã xác thực";
    }
  }
  changeOTP();
...
document
  ...
  .addEventListener("click", MoveToLoginScreen);
